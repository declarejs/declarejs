...
declarejs = (function(){
	...
	makeDatatype = function(type, args){ // not public, added to makers
		...
		if(!datatype) missingError(type);
		if(datatype.hasvalue) return datatype.value;
		datatype.hasvalue = true;
		return datatype.value = datatype.parent ? makeDatatype(datatype.parent) : C_UNDEFINED;
	},
	...
	get = function(name){
		...
		if(window[name]) return classes[name] = window[name];
		return assemble(name).c;
	},
	...
	addMember = function(header, data, struct){
		...
		for(var i=0; i<parts.length; i++){
			...
			switch(part){
				...
				break;
				case "nop": case "noprefill":
					delete struct.preloads[name];
				break;
				case "pre": case "prefill":
					struct.preloads[name] = name;
				...
				default: 
					if(part.length){
						if(debug && member.type) malformedError(header + " ..." + part);
						member.type = part;
					}
	...
	internalError = function(){
		...
		malformedError = internalError,
	missingError = internalError,
	templateParamError = internalError,
	duplicateError = internalError,
	violationError = internalError,
	requiredError = internalError,
	mismatchError = internalError,
	invalidError = internalError,
	...
	declare("abs Base", function(keys, self, parent){return {
		...
		"mix getElse": function(name, _else){
			if(!this["get_" + name]) return _else;
			name = this.get(name);
			return (name === undefined) ? _else : name;
		},
